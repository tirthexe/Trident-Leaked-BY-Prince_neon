...
module.exports = {
  name: "lockall",
  description: "Lock all channels in the server for everyone.",
  async execute(client, message) {
    ...
    if (!message.member.permissions.has(PermissionsBitField.Flags.ManageChannels)) {
      return message.reply({
        embeds: [
          createEmbed(
            "#FF0000",
            `${emoji.error} You don't have permission to lock channels.`,
            executor,
            client,
            null
          ),
    ...
    if (!message.guild.members.me.permissions.has(PermissionsBitField.Flags.ManageChannels)) {
      return message.reply({
        embeds: [
          createEmbed(
            "#FF0000",
            `${emoji.error} I don't have permission to lock channels.`,
            executor,
            client,
            null
          ),
    ...
    setTimeout(async () => {
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId("lockall_yes")
          .setLabel("Yes")
          .setStyle(ButtonStyle.Success),
        new ButtonBuilder()
          .setCustomId("lockall_no")
          .setLabel("No")
      ...
      await initialMessage.edit({
        embeds: [
          createEmbed(
            "#FFFF00",
            `Are you sure you want to lock all channels in this server?`,
            executor,
            client,
            null
          ),
      ...
      const filter = (interaction) =>
        ["lockall_yes", "lockall_no"].includes(interaction.customId);
      ...
      collector.on("collect", async (interaction) => {
        ...
        if (interaction.customId === "lockall_yes") {
          await interaction.deferUpdate();
          await initialMessage.edit({
            embeds: [
              createEmbed(
                "#FFFF00",
                `Locking All Channels ${emoji.loading}`,
                executor,
                client,
                null
              ),
          ...
          for (const channel of channels.values()) {
            try {
              await channel.permissionOverwrites.edit(
                ...
                { SendMessages: false },
                { reason: `Lock all channels triggered by ${executor.tag}` }
              );
            } catch (error) {
          ...
          return initialMessage.edit({
            embeds: [
              createEmbed(
                "#00FF00",
                `${emoji.tick} All channels have been successfully locked.`,
                executor,
                client,
                null
              ),
        ...
        } else if (interaction.customId === "lockall_no") {
          await interaction.deferUpdate();
          return initialMessage.edit({
            embeds: [
              createEmbed(
