...
module.exports = {
  name: "hideall",
  description: "Hide all channels in the server for everyone.",
  async execute(client, message) {
    ...
    setTimeout(async () => {
      ...
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId("hideall_yes")
          .setLabel("Yes")
          .setStyle(ButtonStyle.Danger),
        new ButtonBuilder()
          .setCustomId("hideall_no")
          .setLabel("No")
      ...
      await initialMessage.edit({
        embeds: [
          createEmbed(
            "#FFFF00",
            `Are you sure you want to hide all channels in this server?`,
            executor,
            client,
            null
          ),
      ...
      const filter = (interaction) =>
        ["hideall_yes", "hideall_no"].includes(interaction.customId);
      ...
      collector.on("collect", async (interaction) => {
        ...
        if (interaction.customId === "hideall_yes") {
          // Acknowledge the button press
          await interaction.deferUpdate();

          // Notify that the hiding process is starting
          await initialMessage.edit({
            embeds: [
              createEmbed(
                "#FFFF00",
                `Hiding all channels ${emoji.loading} `,
                executor,
                client,
                null
              ),
          ...
          for (const channel of channels.values()) {
            try {
              await channel.permissionOverwrites.edit(
                ...
                { ViewChannel: false },
                { reason: `Hide all channels triggered by ${executor.tag}` }
              );
            } catch (error) {
          ...
          // Notify that all channels are hidden
          return initialMessage.edit({
            embeds: [
              createEmbed(
                "#00FF00",
                `${emoji.tick} | All channels have been successfully hidden.`,
                executor,
                client,
                null
              ),
        ...
        } else if (interaction.customId === "hideall_no") {
