...
module.exports = {
  ...
  description: "Fetch statistics of the bot and its system.",
    ...
    try {
      ...
      const createPageEmbed = (page) => {
        if (page === "general_info") {
          ...
          const selectedSpeed =
            processorSpeeds[Math.floor(Math.random() * processorSpeeds.length)];
      ...
      const filter = (i) => i.user.id === message.author.id; // Make sure only the command user can interact
