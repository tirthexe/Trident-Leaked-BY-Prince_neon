...
module.exports = {
  name: 'panelpanelmessage',
  aliases: ['ticketpanelmessage' , 'tpm'],
  description: 'Configure the panel message with normal text, embed text, and image.',
        ...
        if (btnInt.customId === 'edit_image') {
          const newImageUrl = await askInput('Enter the new image URL (must be a valid URL):', (url) => {
            try {
              ...
              return urlWithoutQuery.match(/\.(jpeg|jpg|gif|png|webp)$/i);  // Check for image extension
            } catch {
              return false;
            }
          });
          if (newImageUrl) {
            ...
            return btnInt.followUp({ content: 'Invalid image URL. Please try again.' });
          }
        }
