...
module.exports = {
  ...
  execute: async (unban) => {
    ...
    // Fetch the audit log entry for the unban
    ...
    if (executor.id === unban.guild.client.user.id) return;
    ...
    if (executor.id === unban.guild.ownerId || canBanMembers) {
      // If the user is the guild owner or has permission for `ban`
      return;
    }

    // Ban the executor for unauthorized unban action
    try {
      await unban.guild.members.ban(executor.id, { reason: 'Anti-Nuke: Unauthorized member unban' });
    } catch (err) {
      console.error(`Error during anti-unban handling: ${err.message}`);
    }
  },
};